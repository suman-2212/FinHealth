...
def reset_benchmarks():
    """Completely reset the industry_benchmarks table"""
    ...
    try:
        print("Resetting industry_benchmarks table...")
        
        # Drop the table completely
        cursor.execute("DROP TABLE IF EXISTS industry_benchmarks")
        ...
        conn.commit()
        print(f"Successfully reset industry_benchmarks table with {len(default_benchmarks)} benchmark records")
        
        # Verify the table
        cursor.execute("PRAGMA table_info(industry_benchmarks)")
        columns = cursor.fetchall()
        print("Verified table schema:")
        for col in columns:
            print(f"  {col[1]} ({col[2]})")
