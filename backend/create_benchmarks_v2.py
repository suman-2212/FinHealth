...
def create_benchmarks_v2():
    """Create the new industry_benchmarks_v2 table"""
    ...
    try:
        print("Creating industry_benchmarks_v2 table...")
        
        # Create new table with correct schema
        cursor.execute("""
            CREATE TABLE industry_benchmarks_v2 (
                id TEXT PRIMARY KEY,
        ...
        for industry, metric, avg, top, bottom in default_benchmarks:
            cursor.execute("""
                INSERT INTO industry_benchmarks_v2 
        ...
        conn.commit()
        print(f"Successfully created industry_benchmarks_v2 table with {len(default_benchmarks)} benchmark records")
